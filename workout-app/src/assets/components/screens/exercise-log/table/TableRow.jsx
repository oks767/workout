...
const TableRow = ({ item, onChangeTime, getState, toggleTime }) => {
  return (
    <div
    ...
    >
      ...
      <div key={`RepeatWeight ${item?.id}/${item?.weight}`}>
        <input
          ...
          onChange={(e) => onChangeTime(item.id, 'weight', e.target.value)}
        />
        <i>kg{item?.isCompleted && ' '}/</i>
        <input
          type='number'
          ...
          disabled={item?.isCompleted}
          onChange={(e) => onChangeTime(item.id, 'repeat', e.target.value)}
        />
      </div>
